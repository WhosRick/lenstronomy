...
class BPL(LensProfileBase):
    ...
    def set_dynamic(self):
        self._static = False
        for name in ["_b_static", "_a_static", "_a_c_static", "_r_c_static", "_phi_G_static", "_q_static"]:
...
class BPLMajorAxis(LensProfileBase):
    ...
    def function(self, x, y, b, a, a_c, r_c, q):
        vectorized_quad = np.vectorize(
            lambda aa, bb: quad(self.integrand_psi, 0, 1, args=(aa, bb, a, a_c, b, r_c, q))[0]
        )
    ...
    def exhyp2f1(self, a, b, c, z):
        ...
        if c - a - b == 0.5:
            zt = np.sqrt(1 - z)
            fhyp = (2.0 / (1 + zt)) ** (2.0 * a) * hyp2f1(2.0 * a, a - b + 0.5, c, (zt - 1) / (zt + 1))
        else:
            fhyp = hyp2f1(a, b, c, z)
        return fhyp
    ...
    def s0arr(self, alpha, alphac, zel2, c, target_precision):
        ...
        while 1:
            ...
            if nstep < 3:
                ...
                s1 = s2 * 1.0
                s2 = s * 1.0
                aks[sel] = s2[sel] - (s2[sel] - s1[sel]) ** 2.0 / ((s2[sel] - s1[sel]) - (s1[sel] - s0[sel]))
            term = (tc - t) * h
    ...
    def kappa2func(self, b, a, a_c, r_c, R_el):
        if isinstance(R_el, (int, float)):
            ...
            if R_el < r_c:
                return -(3 - a) / self.Beta_func(a) * (b / r_c) ** (a - 1) * z_el * (
                    hyp2f1(a / 2, 1, 1.5, z_el ** 2) - hyp2f1(a_c / 2, 1, 1.5, z_el ** 2)
                )
            else:
                return 0.0
        else:
            ...
            result[mask] = -(3 - a) / self.Beta_func(a) * (b / r_c) ** (a - 1) * z_el_array * (
                hyp2f1(a / 2, 1, 1.5, z_el_array ** 2) - hyp2f1(a_c / 2, 1, 1.5, z_el_array ** 2)
            )
            result[~mask] = 0.0
            return result
    ...
    def integrand_psi(self, u, x, y, alpha, alpha_c, b, r_c, q):
        ...
        return 0.5 * (xi / u) * self.phi_r(xi, alpha, alpha_c, b, r_c) / np.sqrt(1 - (1 - q ** 2) * u)
